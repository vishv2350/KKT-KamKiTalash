...
{
        $Pname      = test_input($_POST["element_4_1"]); //Name = Name of the Person
        $Pmentor    = test_input($_POST["element_4_2"]); //author = Blank
        $Pfile      = test_input($_POST["element_6"]);   //pdf_file = Blank
        $mobile     = test_input($_POST["element_5"]);   //detail_news = Employer Mobile Number
        $e_name     = test_input($_POST["element_6_1"]); //coname = Employer Name
        $e_address  = test_input($_POST["element_6_2"]); //address = Employer Address
        $Rid=0;
        include ("config/conn.php");
                $sql = "SELECT * FROM kkt2 WHERE detail_news='$mobile' LIMIT 1";
                $res = $conn->query($sql);
...
//    $Pid        = 'KKT-0001'; // Short_News
 ...
 //       echo  $Pfile.' Size: ';
     ...
     //   echo  $Pcategory;
        echo "<br />";
        $sql = "INSERT INTO kkt2 ( title,pdf_file,author,short_news,detail_news,`date`,atype,coname,address)
                VALUES('$Pname','$Pfile','$Pmentor','$Pid','$mobile',now(),'$Pcategory','$e_name','$e_address') ";
        include ("config/conn.php");
        $res = $conn->query($sql);
        if ($res) {
...
<body id="main_body" >
   ...
   <!--	<img id="top" src="top.png" alt="">-->
	...
	<div id="form_container">
		...
		<h1><a>KKT - काम की तलाश- EMPLOYER DESK</a></h1>
<?php
if ($_SERVER["REQUEST_METHOD"] <> "POST")
{
?>
		<form id="form_21150"  class="appnitro" enctype="multipart/form-data" method="post" action="">
			...
			<h2>KKT - काम की तलाश- EMPLOYER DESK</h2>
			<p>हम आप को केवल काम करने वालों से लिंक करवायेंग, <BR />हम किसी को भी कारीगर मिलने की गौरानिटी/वचन नहीं देते !</p>
		</div>
			<ul>
        ...
        <li id="li_3" >
    		<label class="description" for="element_6">आप की कंपनी का नाम ? </label>
			<input id="element_6_1" name="element_6_1" class="element text medium" type="element text"  maxlength="150" required/>
            <label class="description" for="element_6_2">आप की कंपनी का एड्रैस ?</label>
            	<div>
			<textarea id="element_6_2" name="element_6_2" class="element textarea medium"></textarea>
		</div>
		</li>

        <li id="li_4" >
		<label class="description" for="element_3">आपको कैसे कारीगर चाइहिए  ? </label>
        <span>
<div class="container">
<div class="row">
<div class="col-sm-2">
